...
1. **AI Knowledge** ➡️ Statis dan internal (ditulis langsung di dalem CRM), berbasis teks atau paragraf, dan gak bisa akomodir data-data yang dinamis (karena ditulis manual).
2. **Integrations** ➡️ Dinamis dan eksternal (masukin link API ke CRM), bisa akomodir data-data dinamis karena data di-_supply_ dari pihak eksternal, skemanya Cekat AI request data tertentu -> pihak eksternal ngembaliin datanya.
...
Seperti yang udah dijelasin sebelumnya, Malika Tools ini bisa dibilang sebuah _middleware_ (atau aplikasi penghubung) antara Cekat AI (aplikasi web juga) ke data-data klien, yang mana bentuk dan formatnya macem-macem. Malika Tools ini bertindak sebagai "mesin pencari" dari _request_ _user_ ke data-data klien yang disambungkan ke Malika Tools. Biar lebih jelas, kita kasih contoh sebuah kasus berikut.
...
2. Cekat AI secara langsung belum punya fitur _connector_ ke data-data dari berbagai platform seperti Google Sheet atau yang lainnya, jadi diperluin _tools_ khusus untuk meng-_handle_ itu semua, yakni si _middleware_ Malika Tools ini
...
1. **Search** ➡️ Digunain buat ngeintegrasiin Malika Tools dengan data-data dari pihak klien dengan platform yang macem-macem
