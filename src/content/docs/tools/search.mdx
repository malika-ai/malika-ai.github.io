...
Buat 1 kolom baru di sebelah paling kiri untuk dijadikan ID dari setiap baris pada data. Penamaan header bebas, boleh ID, boleh PK, boleh yang lain. ID tidak harus angka, boleh string/huruf yang penting unik untuk setiap _row_-nya 
...
#### 3. No dangling spreadsheet item which located in somewhere
...
[Contoh Format Google Sheet](https://docs.google.com/spreadsheets/d/1EqEzhvyO4F5dtmPtX77kZqFgQgsqxuArNh3b8AUbywk)
...
![Alur Membuat _Profile_ Baru](../../../assets/search/create-sheet-flow.png)
<p style="text-align:center;">Alur Membuat _Profile_ Baru</p>
...
Setelah udah buat profil baru (berupa _webhook_) untuk Google Sheet di Malika Tools yang dah dibahas di [Clone and Create New Profile](#clone-and-create-new-profile), kamu bisa daftarin profil baru itu di tools Cekat AI di menu Integrations milik AI Agent.
...
Apabila kalian membuat profil baru di Malika Tools, yang terjadi sebenarnya adalah Malika Tools membuatkan sebuah _webhook_ API agar bisa dipanggil oleh tools yang ada di Cekat AI. Seperti pada webhook biasanya, webhook yang dibuat sama si Malika Tools ini memiliki dua nilai penting: 
...
2. _**Request Body**_ ➡️ Input yang dibutuhkan webhook untuk memproses pencarian. Input ini nantinya didaftarkan di tools Cekat AI pada segmen AI Inputs dan Additional Payload.

_Request body_ yang tersedia untuk webhook pada fitur Search ini ada 4, di mana 2 wajib dan 2 lainnya opsional.
...
Kamu bisa melakukan sinkron di tombol "Sinkronisasi Data" di bawah saat membuka profil di Malika Tools
...
- relation from profile and search features
- directly from tools.malika.ai
- hit from Cekat AI in Integrations tab

### Update and Delete

### Add Row

- sheet must contains existing value in there to append the new data row

## Jubelio

### Credentials Preparation

- the tools need Jubelio creds: username and password

### Clone and Create Profile
